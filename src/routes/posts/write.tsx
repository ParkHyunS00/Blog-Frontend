...
export function PostWritePage(): React.ReactElement | null {
  ...
  const draftId = parsePostId(postIdParam);
  const isInvalidDraftRoute = postIdParam !== undefined && draftId === null;
  const { data: authStatus, isLoading: isAuthLoading } = useAuthStatus();
  const detailQuery = useDraftDetail(draftId);

  if (isAuthLoading) return null;
  if (authStatus?.step !== "AUTHENTICATED") return <Navigate to="/admin" replace />;
  if (isInvalidDraftRoute) return <Navigate to="/admin/posts/write" replace />;
  if (draftId !== null && detailQuery.isError) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-3 px-4 text-center">
        <p className="text-sm text-destructive">임시저장 글을 불러오지 못했습니다.</p>
        <Button variant="outline" size="sm" onClick={() => detailQuery.refetch()}>다시 시도</Button>
      </div>
    );
  }
  if (draftId !== null && !detailQuery.data) return <PostWriteSkeleton />;

  return (
    <PostWriteEditorPage
      key={draftId ?? "new"}
      draftId={draftId}
      initialForm={detailQuery.data ? createFormFromDraft(detailQuery.data) : createEmptyForm()}
    />
  );
}
...
function PostWriteEditorPage({ draftId, initialForm }: { draftId: number | null; initialForm: PostWriteForm }): React.ReactElement {
  ...
  const isBusy = uploadMutation.isPending || saveMutation.isPending || publishMutation.isPending;
  ...
  async function handlePublish(): Promise<void> {
    ...
    try {
      const published = await publishMutation.mutateAsync(result.payload);
      await finalizeSavedImages(form);
      showSuccessToast({ title: "게시글 업로드 완료", description: "게시글이 정상적으로 공개되었습니다." });
      navigate(`/posts/${published.postId}`);
    } catch (error) {
      const fallback = "게시글 발행에 실패했습니다.";
      showErrorToast({ title: "게시글 업로드 실패", error, fallback });
    }
  }
    ...
    <div className="mx-auto w-full max-w-3xl px-4 py-8 md:py-12">
      <div className="space-y-10 md:space-y-12">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3">
          ...
          <Button disabled={isBusy} onClick={handlePublish}>{publishMutation.isPending ? "발행 중..." : "업로드"}</Button>
        </div>
      </div>
    </div>
  );
